...
{
    public ref struct @SkillsDataCenter
    {
        public static ReadOnlyDictionary<BigInteger, SkillData> ReadedSkillsData { get; private set; }
            = ReadOnlyDictionary<BigInteger, SkillData>.Empty;

        public static void ReadSkillsDataFiles()
        {
            Dictionary<BigInteger, SkillData> ReadedThere = [];
            try
            {
                if (Directory.Exists(@"[⇲] Assets Directory\Limbus Images\Skills\Skills Data"))
                {
                    ...
                    {
                        if (SkillsDataFileInfo.TryDeserealizeJsonAs(out LimbusDataFile<SkillData> SkillsDataFile, out Exception Occurred))
                        {
                            foreach (SkillData SkillDataItem in SkillsDataFile.DataList.Where(SkillData => SkillData.ID is not null))
                            {
            ...
            @SkillsDataCenter.ReadedSkillsData = new(ReadedThere);
        }
    ...
    public record SkillData : IHasIdentifier<BigInteger?>
    {
        [JsonProperty("id")]
        ...
        [JsonIgnore]
        public Dictionary<int, SkillData_UptieLevel> UptieLevelsDictionary { get; private set; } = [];
        ...
        public void OnDeserialized(StreamingContext Context)
        {
            ...
            if (UptieLevelsJsonList is not null)
            {
                ...
                ////////////////////////////////////////////////////////////////////
                foreach (SkillData_UptieLevel UptieLevel in UptieLevelsJsonList)
                {
                ...
                SkillData_UptieLevel LatestEncounteredUptie = UptieLevelsDictionary.Values.First();

                //                                   max 6 upties
                for (int UptieNumber = 1; UptieNumber <= 6; UptieNumber++)
                {
                    if (UptieLevelsDictionary.TryGetValue(UptieNumber, out SkillData_UptieLevel? value)) LatestEncounteredUptie = value;
        ...
        public List<SkillData_UptieLevel>? UptieLevelsJsonList { get; set; }

        public record SkillData_UptieLevel
        {
            ...
            public List<SkillData_CoinInfo>? CoinsList { get; set; }

            public record SkillData_CoinInfo
            {
